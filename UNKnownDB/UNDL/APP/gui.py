#!/usr/bin/python3
# -*- encoding: Utf-8 -*-
from PyQt5.QtWidgets import QApplication, QListWidgetItem, QDesktopWidget, QFileDialog, QInputDialog, QProgressDialog, \
    QMessageBox, QFileSystemModel, QHeaderView
...
from utils.common import extractors, transports, BASE_PATH, assert_installed, extractor_save, insert_endpoint, \
    load_proto_msgs
...
class PBTKGUI(QApplication):
    ...
    def extraction_done(self, outputs):
        ...
        if wrote_endpoints:
            ...
            QMessageBox.information(self.view, ' ',
                                    '%d endpoints and their <i>.proto</i> structures have been extracted! You can now reuse the <i>.proto</i>s or fuzz the endpoints.' % nb_written_all)

        elif nb_written_all:
            self.set_view(self.welcome)
            QMessageBox.information(self.view, ' ',
                                    '%d <i>.proto</i> structures have been extracted! You can now reuse the <i>.protos</i> or define endpoints for them to fuzz.' % nb_written_all)
    ...
    def write_endpoint(self):
        ...
        if not (request_pb and urlparse(url).netloc and transport and (not self.has_pb_param or pb_param) and (
            ...
            if sample_data:
                ...
                for sample_id, sample in enumerate(sample_data):
                    try:
                        ...
                        return QMessageBox.warning(self.view, ' ',
            ...
            if type(item) != int:
                if len(data['request'].get('samples', [])) > 1:
                    ...
                    for sample_id, sample in enumerate(data['request']['samples']):
                        self.fuzzer.comboBox.addItem(sample[self.pb_param] if self.pb_param else str(sample),
                                                     (data, sample_id))
